...
function getClass(semester, input, callback1) {
        ...
        request(task, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                ...
                if (nextPagePointsTo.includes(input)) {
                    ...
                    q.push(nextUrl, function(err, result) {
                    })
                }
                ...
                $(classesInfo).each(function(index, tr) {
                    ...
                    $($(tr).children()).each(function(i, td) {
                        ...
                        if(i==1) {
                            ...
                            var cID = $(td).text().replace(/\s/, '').replace(' ', '');
    ...
    // Pushing initial url
    q.push(url, function(err, result) {
    })
}

// Testing
getClass("20174", "CASCH102", function(err, result) {
    if (err) {
        console.log(err);
        return;
    }
...
// getClasses("20174", ["CASCS111", "QSTSM131", "CASWR100"], function(err, result) {
//     if(err) {
//         console.log(err);
//     }
//     console.log(JSON.stringify(result));
