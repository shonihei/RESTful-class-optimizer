...
function getClasses(semester, arrayOfClasses, done) {
    async.map(arrayOfClasses, getClass.bind(null, semester), function(err, result) {
        if(err) {
            done("Something went terribly wrong and the developer has not handled this error");
        }
        var newClasses = new Object();
        async.each(result, function(data, next) {
            newClasses[data.courseID] = data;
            next();
        }, function(err) {
            if(!err) {
                done(null, newClasses);
            }
        })
    })
}
